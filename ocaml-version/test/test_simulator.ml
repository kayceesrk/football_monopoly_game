...
let simulate_game () =
  ...
  let rec play_turns state turn_count max_turns properties youths stars =
    if turn_count >= max_turns then
      (None, turn_count, properties, youths, stars)
    else
      (* Roll dice *)
      ...
      | Game_types.Winner winner ->
          (Some winner.Game_types.name, turn_count + 1, properties, youths, stars)
      | Game_types.Continue new_state ->
          ...
          | Game_types.Winner winner ->
              (Some winner.Game_types.name, turn_count + 1, new_properties, new_youths, new_stars)
          | Game_types.Continue next_state ->
              play_turns next_state (turn_count + 1) max_turns new_properties new_youths new_stars
          | Game_types.Error _ ->
              (None, turn_count, new_properties, new_youths, new_stars))
      | Game_types.Error _ ->
          (None, turn_count, properties, youths, stars)
  in

  play_turns state 0 1000 0 0 0
...
let run_simulations num_games =
  ...
  for game_num = 1 to num_games do
    let (winner, turns, props, youths, stars) = simulate_game () in
